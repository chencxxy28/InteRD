...
for(iter_rep in 1:overall)
{
  ...
  comb_sampled<-matrix(1/4,nrow=samplesize,ncol=4)
  ...
  lambda_option<-0
  MURD.onegroup<-function (pseudo.seger,list_marker,cell_type_unique,comb_sampled,ave_est,ave_sd,lambda_option){
    bulk.eset<-pseudo.seger
    bulk_data<-(exprs(bulk.eset))
    marker_gene_used<-unlist(list_marker)
    gene_test<-setdiff(rownames(bulk_data),marker_gene_used)
    ...
    for(ll in 1:length(lambda_option))
    {
      ...
      repeat{
        prop_new<-t(sapply(1:ncol(bulk_nozero), function (x)
        {
          ...
          X_aug<-sqrt(lambda_adjust)*diag(1,length(ct.sub))/ave_sd
  ...
  results_1<-MURD.onegroup(pseudo.seger=pseudo.seger$pseudo_eset,list_marker,cell_type_unique,comb_sampled=comb_sampled,ave_est,ave_sd,lambda_option)
  musec_1<-results_1$musec
  prop_est<-results_1$est[[which.min(musec_1)]]
  
  ref_free<-cbind(ref_free,prop_est)
  truep_all<-cbind(truep_all,truep)
  print(iter_rep)
}

#change here
write.csv(ref_free,file="/storage/home/bzs438/chixiang/deconvolution/interd2_new/xinscrna/ref_free/ref_freen80m20.csv")
write.csv(truep_all,file="/storage/home/bzs438/chixiang/deconvolution/interd2_new/xinscrna/ref_free/ref_freen80mr20.csv")
...
# names(mad_eachrun)<-rep(c("alpha","beta","delta","gamma"),times=10)
...
# }
# ))
# names(cor_eachrun)<-rep(c("alpha","beta","delta","gamma"),times=10)
